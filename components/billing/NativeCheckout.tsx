...
export function NativeCheckout({ planCode, cycle, label }: { planCode: string; cycle: "monthly" | "annual"; label: string }) {
  ...
  async function loadPending() {
    ...
    setReady(false); setChecking(true); setError(""); setQuote(null); setPayment(null);
    try {
    ...
    finally { if (version === view.current) setChecking(false); }
  }
  async function checkPayment(id: string) {
    ...
    } catch (e) { if (version === view.current) setError(e instanceof Error ? e.message : "Confirmação pendente. Verifique novamente."); }
  }
  ...
  async function pay(event: FormEvent<HTMLFormElement>) {
    ...
    if (!ready || busy || pending || submitting.current) return;
    if (!quote) { await calculate(); return; }
  ...
  return <Dialog open={open} onOpenChange={(value) => {
    ...
    else { view.current++; setQuote(null); setBusy(false); }
  }}>
    ...
    <DialogContent className="max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-[460px] overflow-y-auto overscroll-contain rounded-2xl p-5 sm:p-6 duration-150 motion-reduce:animate-none" onInteractOutside={(e) => { if (submitting.current) e.preventDefault(); }} onEscapeKeyDown={(e) => { if (submitting.current) e.preventDefault(); }}>
      <DialogHeader className="pr-6 text-left"><DialogTitle>Renovar assinatura</DialogTitle><DialogDescription>Plano {cycle === "annual" ? "anual" : "mensal"}. Preencha seus dados e confira o total antes de confirmar.</DialogDescription></DialogHeader>
      ...
      {!ready && <div className="text-xs text-muted-foreground" role="status">{checking ? "Verificando pagamentos anteriores. Você já pode preencher seus dados." : <Button onClick={() => void loadPending()} variant="outline">Tentar novamente</Button>}</div>}
      ...
      {!pending && payment?.status !== "paid" && <div className="min-w-0 space-y-4">
        ...
        <form onSubmit={pay} className="sentry-block space-y-4" data-sentry-block>
          <fieldset disabled={busy} className="grid min-w-0 grid-cols-2 gap-3">
            <legend className="sr-only">Dados do pagador</legend>
            <div className="col-span-2"><Field name="name" label="Nome completo / razão social" autoComplete="name" /></div>
            <div className="col-span-2"><Field name="email" label="E-mail" type="email" autoComplete="email" /></div>
          ...
          <fieldset disabled={busy} className="space-y-3"><legend className="mb-2 text-sm font-medium">Como deseja pagar?</legend><div className="grid grid-cols-2 gap-3">
            <button type="button" aria-pressed={method === "PIX"} className={`min-h-14 rounded-lg border p-3 text-sm ${method === "PIX" ? "border-primary bg-primary/10" : "bg-muted/30"}`} onClick={() => { setMethod("PIX"); setInstallments(1); setQuote(null); }}>Pix</button>
            <button type="button" disabled={!cardEnabled} aria-pressed={method === "CREDIT_CARD"} className={`min-h-14 rounded-lg border p-3 text-sm disabled:opacity-50 ${method === "CREDIT_CARD" ? "border-primary bg-primary/10" : "bg-muted/30"}`} onClick={() => { setMethod("CREDIT_CARD"); setQuote(null); }}>Cartão</button>
          </div>{!cardEnabled && <p className="text-xs text-muted-foreground">{checking ? "Verificando disponibilidade do cartão…" : "Cartão indisponível neste momento. Pix disponível neste fluxo."}</p>}
          {method === "CREDIT_CARD" && <label className="block text-sm">Parcelas<select value={installments} onChange={(e) => { setInstallments(Number(e.target.value)); setQuote(null); }} className="mt-1 h-10 w-full rounded-md border bg-background px-3">{Array.from({ length: cycle === "monthly" ? 1 : 12 }, (_, n) => <option key={n} value={n + 1}>{n + 1}x — total calculado antes de confirmar</option>)}</select></label>}</fieldset>
          ...
          <section className="rounded-lg border bg-muted/30 p-4 text-sm" aria-label="Resumo do pagamento"><h3 className="break-words font-semibold">{quote.plan_name} · {cycle === "annual" ? "Anual" : "Mensal"}</h3>
            ...
            <Button type="submit" disabled={!ready || busy} className="h-auto min-h-11 w-full whitespace-normal px-3 py-2">{busy ? "Calculando…" : !quote ? "Avançar para pagamento" : method === "PIX" ? `Gerar Pix de ${money(quote.total_cents)}` : `Confirmar pagamento de ${money(quote.total_cents)}`}</Button>
...
function Field({ label, ...props }: ComponentProps<typeof Input> & { label: string }) {
  return <label className="block min-w-0 text-[11px] font-medium uppercase tracking-wide">{label}<Input required autoComplete="off" {...props} className="mt-1 min-h-10 min-w-0 bg-muted/30 text-base font-normal normal-case tracking-normal sm:text-sm" /></label>;
}
