  ...
  if (!capacity.allowed) {
    ...
    return fail(
      ...
      capacity.reason === "plan_limit_reached"
        ? t(`Seu plano permite até ${capacity.limit} número(s) de WhatsApp.`)
        : capacity.reason === "subscription_inactive"
          ? t("A assinatura desta empresa não permite conectar novos números.")
          : t("Não foi possível validar os limites do plano agora."),
      status,
